from __future__ import annotations

import json
import threading
import uuid
from datetime import datetime
from pathlib import Path
from typing import Any, Dict, Optional, Tuple

from sqlalchemy import select

from app.core.config import get_settings
from app.core.performance import correlation_scope, emit_performance_event
from app.db.models import ModelRun, TrainingTask
from app.db.session import session_scope
from app.training.train import run_training


def _execute_training_task(task_id: str) -> None:
    started_at = datetime.utcnow()
    payload: Dict[str, Any] = {}

    with session_scope() as session:
        task = session.get(TrainingTask, task_id)
        if task is None:
            return
        task.status = "running"  # type: ignore[assignment]
        task.started_at = started_at
        task.error_message = None
        payload = dict(task.request_payload or {})

    correlation_id = (str(payload.get("correlation_id") or "").strip() or None)

    with correlation_scope(correlation_id):
        config_payload = payload.get("config")
        config_path: Optional[Path] = None
        if isinstance(config_payload, dict):
            settings = get_settings()
            cfg_dir = Path(settings.data_root) / "async_train_configs"
            cfg_dir.mkdir(parents=True, exist_ok=True)
            config_path = cfg_dir / f"{task_id}.json"
            config_path.write_text(json.dumps(config_payload), encoding="utf-8")

        try:
            run_id, _ = run_training(config_path=config_path)
            completed_at = datetime.utcnow()
            with session_scope() as session:
                task = session.get(TrainingTask, task_id)
                if task is None:
                    return
                task.status = "success"  # type: ignore[assignment]
                task.completed_at = completed_at
                task.run_id = run_id
                task.error_message = None

                run = session.get(ModelRun, run_id)
                if run is not None:
                    data_manifest = dict(run.data_manifest or {})
                    data_manifest["async_context"] = {
                        "task_id": task_id,
                        "queue_name": str(task.queue_name),
                        "correlation_id": correlation_id,
                    }
                    run.data_manifest = data_manifest

            emit_performance_event(
                "training.async.success",
                task_id=task_id,
                run_id=run_id,
                duration_ms=(completed_at - started_at).total_seconds() * 1000.0,
            )
        except Exception as exc:  # noqa: BLE001
            completed_at = datetime.utcnow()
            with session_scope() as session:
                task = session.get(TrainingTask, task_id)
                if task is None:
                    return
                task.status = "failed"  # type: ignore[assignment]
                task.completed_at = completed_at
                task.error_message = f"{type(exc).__name__}: {exc}"
            emit_performance_event(
                "training.async.failed",
                task_id=task_id,
                error_type=type(exc).__name__,
                duration_ms=(completed_at - started_at).total_seconds() * 1000.0,
            )


def _dispatch_training_task(task_id: str) -> Optional[str]:
    settings = get_settings()
    if settings.celery_enabled:
        try:
            from celery import Celery

            app = Celery(
                "predictive_deterministic_model",
                broker=settings.celery_broker_url,
                backend=settings.celery_result_backend,
            )
            app.conf.task_default_queue = settings.queue_training_name
            app.conf.task_always_eager = settings.celery_task_always_eager

            async_result = app.send_task(
                "app.worker.execute_training_task",
                args=[task_id],
                queue=settings.queue_training_name,
            )
            return str(async_result.id)
        except Exception as exc:  # noqa: BLE001
            if not settings.async_fallback_sync_execution:
                raise RuntimeError(f"Celery dispatch failed: {type(exc).__name__}: {exc}") from exc

    thread = threading.Thread(target=_execute_training_task, args=(task_id,), daemon=True)
    thread.start()
    return None


def enqueue_training_task(
    config_payload: Optional[Dict[str, Any]],
    idempotency_key: Optional[str],
    correlation_id: Optional[str] = None,
    principal_context: Optional[Dict[str, Any]] = None,
) -> Tuple[str, bool]:
    normalized_key = (idempotency_key or "").strip() or None

    with session_scope() as session:
        if normalized_key is not None:
            existing = (
                session.execute(
                    select(TrainingTask)
                    .where(TrainingTask.idempotency_key == normalized_key)
                    .order_by(TrainingTask.created_at.desc(), TrainingTask.task_id.desc())
                )
                .scalars()
                .first()
            )
            if existing is not None:
                return existing.task_id, True

        settings = get_settings()
        task_id = uuid.uuid4().hex
        task = TrainingTask(
            task_id=task_id,
            status="pending",  # type: ignore[assignment]
            queue_name=settings.queue_training_name,
            idempotency_key=normalized_key,
            request_payload={
                "config": config_payload or {},
                "correlation_id": (correlation_id or "").strip() or None,
                "principal_context": dict(principal_context or {}),
            },
        )
        session.add(task)

    celery_task_id: Optional[str] = None
    dispatch_error: Optional[str] = None
    try:
        celery_task_id = _dispatch_training_task(task_id)
    except Exception as exc:  # noqa: BLE001
        dispatch_error = f"{type(exc).__name__}: {exc}"

    if celery_task_id is not None or dispatch_error is not None:
        with session_scope() as session:
            task = session.get(TrainingTask, task_id)
            if task is not None:
                task.celery_task_id = celery_task_id
                if dispatch_error is not None:
                    task.status = "failed"  # type: ignore[assignment]
                    task.completed_at = datetime.utcnow()
                    task.error_message = dispatch_error

    emit_performance_event(
        "training.async.enqueued",
        task_id=task_id,
        queue_name=get_settings().queue_training_name,
        idempotency_key=normalized_key,
        correlation_id=(correlation_id or "").strip() or None,
        idempotent_reused=False,
        celery_enabled=get_settings().celery_enabled,
    )
    return task_id, False


def get_training_task(task_id: str) -> Optional[TrainingTask]:
    with session_scope() as session:
        return session.get(TrainingTask, task_id)
